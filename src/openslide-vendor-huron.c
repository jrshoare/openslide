...
static bool huron_open(openslide_t *osr,
  ...
  /*
   * The first image in a Huron file is always the baseline image (full
   * resolution). This image is always tiled, usually with a tile size
   * of 512 x 512 pixels. The second image is always a thumbnail,
   * typically with dimensions of about 1415 x 3221 pixels. Unlike the
   ...
   *
   * Optionally at the end of a Huron file there may be a slide label
  ...
  do {
    ...
    if (TIFFIsTiled(tiff)) {
      ...
      // some slides may have some zero-length tiles, apparently due to
      // an encoder bug
      toff_t *tile_sizes;
